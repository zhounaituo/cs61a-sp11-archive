...
; data by constructing it from two branches (for example, using 
; list):
...
(define (total-weight bm)
    (cond ((null? bm) 0)
          ((not (pair? (car bm))) (car bm))
          (else (+ (total-weight (car bm))
                   (total-weight (cdr bm))))))

; c. A mobile is said to be balanced if the torque applied by
; its top-left branch is equal to that applied by its top151
; right branch (that is, if the length of the left rod multiplied 
; by the weight hanging from that rod is equal to the corresponding 
; product for the right side) and if each of the submobiles hanging 
; off its branches is balanced. Design a predicate that tests 
; whether a binary mobile is balanced.

(define (ba bm)
    (cond ((null? bm) #t)
          ((not (pair? (car bm))) )
          (else (and (ba (car bm)) (ba (cdr bm))))))
